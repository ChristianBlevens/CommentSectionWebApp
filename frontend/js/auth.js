...
const Auth = {
    ...
    buildApiUrl(endpoint) {
        return window.buildApiUrl ? window.buildApiUrl(endpoint) : (this.getBasePath() + '/api' + endpoint);
    },
    ...
    async checkExistingSession() {
        ...
        try {
            ...
            try {
                const response = await fetch(this.buildApiUrl('/session/validate'), {
                    headers: {
    ...
    // Logout user
    async signOut(apiUrl = null) {
        const sessionToken = localStorage.getItem('auth_token');
        if (sessionToken) {
            try {
                await fetch(this.buildApiUrl('/logout'), {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${sessionToken}`
                    }
